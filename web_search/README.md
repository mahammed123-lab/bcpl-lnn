# JARVIS Web Search (v7.1)

Layer 2 of the search_gate pipeline for Hussain's personal AI assistant, JARVIS.
...
- **Circuit breaker everywhere**: any provider (search or LLM) that fails `JARVIS_FAILURE_THRESHOLD` times in a row is put on cooldown for `JARVIS_COOLDOWN_SECONDS`, so a dead/rate-limited key doesn't eat a timeout on every single query.
...
| Tuning | `JARVIS_FAILURE_THRESHOLD` / `JARVIS_COOLDOWN_SECONDS` | circuit breaker |
...
(e.g. JARVIS's own `orchestra.py` brain), just pass `call_llm`:
...
5. **Optional but recommended:**
   ...
   - If this will be imported into your main JARVIS repo as a
